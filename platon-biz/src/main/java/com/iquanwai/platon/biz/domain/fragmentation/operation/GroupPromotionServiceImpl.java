...
public class GroupPromotionServiceImpl implements GroupPromotionService {
    ...
    private void sendParticipateSuccessMessage(Integer newProfileId, String groupCode) {
        ...
        // 给新人发送消息
        if (remainderCount > 0) {
            String newProfileMessage = "你已接受" + leaderProfile.getNickname() + "邀请，还差" + remainderCount + "人加入解锁7天课程内容，请等待解锁成功通知。你可以<a href='"
        ...
        if (remainderCount > 0) {
            ...
            data.put("first", new TemplateMessage.Keyword(newProfile.getNickname() + "已接受邀请。还差最后" + remainderCount + "人加入，" + GROUP_PROMOTION_SUCCESS_COUNT + "人免费解锁前7天课程内容。\n"));
