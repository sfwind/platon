...
public class SubjectArticleDao extends PracticeDBUtil {
    ...
    public List<SubjectArticle> loadBatchSubjects(Integer problemId,List<Integer> refers){
        ...
        String sql = "select * from SubjectArticle where  ProblemId = ? and Id in (" + mask + ")";

        try{
            return runner.query(sql, new BeanListHandler<SubjectArticle>(SubjectArticle.class), params.toArray());
        } catch (SQLException e) {
            logger.error(e.getLocalizedMessage(), e);
        }
        return Lists.newArrayList();
